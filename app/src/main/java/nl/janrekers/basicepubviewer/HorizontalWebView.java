...
public class HorizontalWebView extends WebView {
    ...
    private ChapterMover chaperMover;

    public HorizontalWebView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public void setChapterMover(ChapterMover cm) {
        chaperMover = cm;
    }
    ...
    private void turnPageLeft() {
        if (getCurrentPage() > 0) {
            ...
            chaperMover.loadNextOrPrevChapter(-1);
        }
    }
    ...
    public void moveToLastPage() {
        // @TODO dit werkt nog niet goed, beter uitzoeken hoe dat moet
        int scrollX = this.getMeasuredWidth();
        loadAnimation(scrollX);
        current_x = scrollX;
        scrollTo(scrollX, 0);
    }
